...
//DOES WORK - but when enabled effect to a player with effecttype it gives diffferent effect. eg. enable effect effecttype.burned - but player gets effecttype.amnesiavision - no idea waiting for newer exiled version.
...
public class Scp714 : CustomItem
{
    ...
    private void OnReceivingEffect(ReceivingEffectEventArgs ev)
    {
        ...
        {
            Log.Debug("Contains this effect!");
            ev.IsAllowed = false;
        }
    }
    ...
    private void sSetRingState(Player player, bool equipped)
    {
        switch (equipped)
        {
            case true:
