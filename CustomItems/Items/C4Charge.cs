...
public class DetectCollision : MonoBehaviour
{
    private void OnCollisionEnter(Collision collision)
    {
        if (collision == null)
            return;
        collision.contacts.FirstOrDefault().thisCollider.attachedRigidbody.isKinematic = true;
        Log.Debug($"Grenade collided with: {collision.gameObject.name} and sticked to the surface!");
    }
}
...
public class C4Charge : CustomGrenade
{
    ...
    /// <inheritdoc/>
    public override string Name { get; set; } = "Semtex C4";
    ...
    protected override void OnThrownProjectile(ThrownProjectileEventArgs ev)
    {
        ...
        if (IsSticky)
            ev.Projectile.GameObject.AddComponent<DetectCollision>();
